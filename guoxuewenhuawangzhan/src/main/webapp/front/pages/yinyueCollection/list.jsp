            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">音乐收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="yinyueName" id="yinyueName" placeholder="音乐名" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../yinyue/detail.jsp?id='+item.yinyueId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.yinyuePhoto?item.yinyuePhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../yinyue/detail.jsp?id='+item.yinyueId)"><span>{{item.yinyueName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.yinyueNewMoney"><span style="color: red;">{{item.yinyueNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteYinyueCollection(item.id)">删除 {{item.yinyueName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 yinyueCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            yinyueCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteYinyueCollection(id){
                var paramArray = [];
                paramArray.push(id);
                layui.http.requestJson(`yinyueCollection/delete`, 'post',paramArray, function (res) {
                    if (res.code == 0) {
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#yinyueName').val() != null && jquery('#yinyueName').val() != "" && jquery('#yinyueName').val() != "null" ) {
                param['yinyueName'] = jquery('#yinyueName').val();
            }
                                                                
            // 当前表
                                // 获取列表数据
            http.request('yinyueCollection/list', 'get', param, function(res) {

            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">舞蹈收藏展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">舞蹈名</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="wudaoName" id="wudaoName" placeholder="舞蹈名" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../wudao/detail.jsp?id='+item.wudaoId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.wudaoPhoto?item.wudaoPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../wudao/detail.jsp?id='+item.wudaoId)"><span>{{item.wudaoName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.wudaoNewMoney"><span style="color: red;">{{item.wudaoNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteWudaoCollection(item.id)">删除 {{item.wudaoName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 wudaoCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            wudaoCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteWudaoCollection(id){
                var paramArray = [];
                paramArray.push(id);
                layui.http.requestJson(`wudaoCollection/delete`, 'post',paramArray, function (res) {
                    if (res.code == 0) {
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#wudaoName').val() != null && jquery('#wudaoName').val() != "" && jquery('#wudaoName').val() != "null" ) {
                param['wudaoName'] = jquery('#wudaoName').val();
            }
                                                                
            // 当前表
                                // 获取列表数据
            http.request('wudaoCollection/list', 'get', param, function(res) {
